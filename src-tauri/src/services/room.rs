...
//     let room_model = rooms::Entity::find()
//         .filter(
//             Condition::any()
//                 .add(rooms::Column::Creator.eq(user_uuid.to_string()))
//                 .add(rooms::Column::Admins.contains(&user_uuid.to_string())),
//         )
//         .filter(rooms::Column::Id.eq(room_uuid))
//         .one(db)
//         .await?
